...
class LSTM(nn.Module):
    def __init__(
        ...
        self.class_name = f'{"Bi" if is_bidirectional else ""}LSTM{"WithAttention" if has_attention else ""}With{embedding_type}Embeddings'

        self.has_attention = has_attention
        self.embedding_type = embedding_type

        # Embedding layer for translating tokens into a form that captures linguistic features suitable for downstream tasks like classification
        if self.embedding_type == 'bert':
            # Load pre-trained BERT model
            _bert_model = BertModel.from_pretrained('bert-base-uncased')
            self.embedding_layer = _bert_model.embeddings
            input_size = _bert_model.config.hidden_size
        else:
            # Initialize embedding layer with pre-trained GloVe or Word2Vec embeddings
            self.embedding_layer = nn.Embedding(len(vocab), embedding_dim, padding_idx=0)
    ...
    def forward(self, input_ids):
        # Generate embeddings for given input_ids to map the token IDs to continuous-valued embeddings
    ...
    def load_glove_embeddings(path, vocab, embedding_dim):
        ...
        with open(path, 'r', encoding='utf-8') as f:
            for line in f:
                ...
                if word in vocab:
                    idx = vocab[word]
                    _embeddings[idx] = vector
        return torch.tensor(_embeddings, dtype=torch.float32)
